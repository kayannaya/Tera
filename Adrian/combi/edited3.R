...
counts <- read.table("/home/bacteraia/combi/06_DESeq2/counts_3.txt", header = TRUE, row.names = 1, sep = "\t")
head(counts)
...
res <- results(dds)
head(res)
write.csv(as.data.frame(res), file = "/home/bacteraia/combi/06_DESeq2/deseq2_results_unfiltered_3.csv")

res_filtered <- res[which(res$padj < 0.05), ]
head(res_filtered)
write.csv(as.data.frame(res_filtered), file = "/home/bacteraia/combi/06_DESeq2/deseq2_results_filtered_3.csv")
